...
def test(model, test_dl, epoch, loss_obj=FocalLoss(), edge_features=True, plot=True, save=True, output_folder=None, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
    ...
    with torch.set_grad_enabled(False):
        ...
        for sample in tqdm(test_dl, desc="Validation"):
            ...
            if edge_features:
                #print(" sample.edge_index.shape[1] ", sample.edge_index.shape[1], " sample.edge_attr.shape[0] ", sample.edge_attr.shape[0])
...
def manual_likelihood_ratios(y_true, y_pred):
    ...
    # Likelihood Ratios
    pos_lr = tpr / (1 - tnr) if (1 - tnr) > 0 else float('inf')
    neg_lr = (1 - tpr) / tnr if tnr > 0 else float('inf')
    
    return pos_lr, neg_lr
...
def plot_validation_results(pred, y, save=True, output_folder=None, file_suffix=None, ax=None):
    ...
    if save and output_folder is not None and file_suffix is not None:
        ax.savefig(f"{output_folder}/{file_suffix}.png", dpi=300,bbox_inches='tight', transparent=True)
'''
...
def plot_validation_results(pred, y, save=True, output_folder=None, file_suffix=None, ax=None):
    ...
    if ax is None:
        ...
        fig = ax[0, 0].figure  # Try to get the figure from the first subplot

    _, TNR, TPR, _, thresholds = classification_threshold_scores(pred, y, ax[0, 0], threshold_step=0.05)
    #plot_confusion_matrix(pred, y, ax[2, :])
    best_threshold = get_best_threshold(TNR, TPR, thresholds)
    plot_confusion_matrix(pred, y, ax[2, :], best_threshold)
    plot_prediction_distribution(pred, y, ax[3:6, :], thres=best_threshold)
    plot_edge_distribution(pred, y, ax[1, :])
    plot_roc_curve(pred, y, ax[0, 1], best_threshold)
    ...
    if save:
        fig.savefig(f"{output_folder}/{file_suffix}.png", dpi=300, bbox_inches='tight', transparent=True)

    return fig, ax if created_fig else None
...
def get_best_threshold(TNR, TPR, thresholds, default=0.65):
    # Compute absolute differences between TNR and TPR
    diffs = [abs(tnr - tpr) for tnr, tpr in zip(TNR, TPR)]
    min_index = int(np.argmin(diffs))
    
    # If the minimum difference is acceptable, return corresponding threshold
    if diffs[min_index] < 0.05:  # configurable tolerance
        return round(thresholds[min_index], 3)
...
def print_acc_scores(pred, y, thres=0.65):
    ...
    pos_lr, neg_lr = manual_likelihood_ratios(y_discrete, pred_discrete)
...
def plot_confusion_matrix(pred, y, axes, thres=0.65):
    ...
    axes[1].set_title(f"Normalised. Threshold: {thres}", fontsize=14)
