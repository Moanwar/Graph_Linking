...
class ClusterDataset(Dataset):
    ...
    def __init__(self, root, histo_path, transform=None, test=False, pre_transform=None, pre_filter=None):
        ...
        # Collect all processed files once
        self.all_files = sorted(glob(osp.join(self.processed_dir, "data_*.pt")))
    ...
    def get(self, idx):
        ...
        try:
            ...
            return None  # skip if file cannot be loaded
    ...
    def download(self):
        if (self.test):
            ...
            print(" self.histo_path ", self.histo_path)
            files = glob(f"{self.histo_path}/*.root")
            print("working")
        for id in range(len(files)):
            ...
            tsCP=allsimtrackstersCP.arrays(["trackIdx","regressed_energy","regressed_pt","pdgID"])
